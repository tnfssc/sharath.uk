...
export const SparklesText: React.FC<SparklesTextProps> = ({
  ...
  useEffect(() => {
    const generateStar = (): Sparkle => {
      ...
      const lifespan = Math.random() * 1 + 2;
      const id = `${sparklesId}-${starX}-${starY}-${Date.now()}`;
      return { id, color, delay, scale, x: starX, y: starY, lifespan };
    };
  ...
  }, [colors.first, colors.second, sparklesCount, sparklesId]);
...
const Sparkle: React.FC<Sparkle> = ({ x, y, id, color, delay, scale }) => {
  return (
    <motion.svg
      key={id}
      width="21"
      height="21"
      viewBox="0 0 21 21"
      initial={{ top: y, left: x, opacity: 0 }}
